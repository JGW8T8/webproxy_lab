...
void doit(int clientfd)
{
  ...
  sscanf(request_buf, "%s %s", method, uri);   // "GET /index.html" → method, uri 분리
  parse_uri(uri, hostname, port, path);        // URI → 호스트명, 포트, 경로로 분리
  ...
  // 캐시 확인 (LRU 캐시 정책 사용)
  //LRU (Least Recently Used): 가장 오래전에 사용된 데이터를 가장 먼저 제거
  CachedObject *cached_object = find_cache(path);
  if (cached_object) // 캐시 히트
  {
  ...
  // 첫 줄 재구성 후 서버로 전송
  sprintf(request_buf, "%s %s HTTP/1.0\r\n", method, path);     // HTTP 1.0으로 재설정 
  Rio_writen(serverfd, request_buf, strlen(request_buf));       // 서버로 전송
  ...
  // 응답 본문 수신 및 전송
  response_ptr = malloc(content_length);                    // 응답 바디 동적 할당
  Rio_readnb(&response_rio, response_ptr, content_length);  // 바디 읽기
  Rio_writen(clientfd, response_ptr, content_length);       // 클라이언트로 전송

  // 캐싱 가능한 경우 캐시에 저장
  if (content_length <= MAX_OBJECT_SIZE) 
  {
    CachedObject *Cache = (CachedObject *)calloc(1, sizeof(CachedObject));
    Cache->response_ptr = response_ptr;       // 캐시 객체 본문 저장
    Cache->content_length = content_length;   // 캐시 객체에 본문 길이 저장
    strcpy(Cache->path, path);                // 경로값을 캐시키로 설정
    write_cache(Cache);                       // 캐시 테이블에 넣기 
  }
